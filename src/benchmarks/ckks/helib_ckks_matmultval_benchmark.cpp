...
MatMultValBenchmarkDescription::MatMultValBenchmarkDescription()
{
    ...
    default_workload_params.add<std::uint64_t>(
        MatMultValBenchmarkDescription::DefaultPolyModulusDegree,
        "PolyModulusDegree");
    default_workload_params.add<std::uint64_t>(
    ...
    default_workload_params.add<std::uint64_t>(
        ...
        "KeySwitchColumns");
    default_workload_params.add<std::uint64_t>(
        MatMultValBenchmarkDescription::DefaultPrecision, "Precision");
    ...
    const hebench::APIBridge::WorkloadParams *p_w_params) const
{
    ...
    std::uint64_t poly_modulus_degree =
        p_w_params
            ->params[MatMultValBenchmarkDescription::Index_PolyModulusDegree]
            .u_param;
    ...
    std::uint64_t precision =
        p_w_params->params[MatMultValBenchmarkDescription::Index_Precision]
            .u_param;
    ...
    m_w_params(bench_params)
{
    ...
    if (bench_desc.workload != hebench::APIBridge::Workload::MatrixMultiply || bench_desc.data_type != hebench::APIBridge::DataType::Float64 || bench_desc.category != hebench::APIBridge::Category::Latency || ((bench_desc.cipher_param_mask & 0x03) != 0x03) || bench_desc.scheme != HEBENCH_HE_SCHEME_CKKS || bench_desc.security != HEBENCH_HE_SECURITY_128 || bench_desc.other != MatMultValBenchmarkDescription::MatMultValOtherID)
        throw hebench::cpp::HEBenchError(
    ...
    std::uint64_t precision = m_w_params.get<std::uint64_t>(
        MatMultValBenchmarkDescription::Index_Precision);
    ...
    assert(m_p_ctx_wrapper->getSlotCount() == poly_modulus_degree / 2); // check on this ****
}
...
std::vector<std::vector<std::double_t>> MatMultValBenchmark::prepareMatrix(
    const hebench::APIBridge::NativeDataBuffer &buffer, std::uint64_t rows,
    std::uint64_t cols)
{
    std::vector<std::vector<std::double_t>> retval(
        rows, std::vector<std::double_t>(cols));
    if (!buffer.p || buffer.size < rows * cols * sizeof(std::double_t))
        throw hebench::cpp::HEBenchError(
            HEBERROR_MSG_CLASS("Insufficient data for M0."),
            HEBENCH_ECODE_INVALID_ARGS);
    const std::double_t *p_curr_row =
        reinterpret_cast<const std::double_t *>(buffer.p);
...
std::vector<helib::PtxtArray> MatMultValBenchmark::encodeMatrix(
    const std::vector<std::vector<std::double_t>> &data)
{
    ...
    const std::vector<std::vector<std::double_t>> &data)
{
    ...
    // transpose
    std::vector<std::vector<std::double_t>> data_T(
        m_w_params.cols_M1(), std::vector<std::double_t>(m_w_params.cols_M0()));
    ...
    const hebench::APIBridge::DataPackCollection *p_parameters)
{
    ...
    std::vector<std::vector<std::double_t>> matrix_data =
    ...
    if (data_pack_index < p_native->pack_count)
    {
        ...
        if (result_component.buffer_count > 0 && result_component.p_buffers)
        {
            ...
            if (buffer.p && buffer.size > 0)
            {
                ...
                std::double_t *p_data = reinterpret_cast<std::double_t *>(buffer.p);
                for (std::size_t row_i = 0; row_i < m_w_params.rows_M0(); ++row_i)
                {
                    for (std::size_t col_i = 0; col_i < m_w_params.cols_M1(); ++col_i)
                    {
                        std::vector<std::double_t> decoded;
                        decoded = m_p_ctx_wrapper->decodeCKKS(encoded_result[row_i][col_i]);
