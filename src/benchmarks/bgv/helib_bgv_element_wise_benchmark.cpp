...
ElementWiseBenchmarkDescription::ElementWiseBenchmarkDescription(
    hebench::APIBridge::Category category, hebench::APIBridge::Workload op)
{
    if (op != hebench::APIBridge::Workload::EltwiseAdd && op != hebench::APIBridge::Workload::EltwiseMultiply)
        throw hebench::cpp::HEBenchError(
            HEBERROR_MSG_CLASS("Workload operation not supported."),
    ...
    case hebench::APIBridge::Category::Offline:
        m_descriptor.cat_params.offline.data_count[0] = 5; // flexibl
    ...
    default_workload_params.add<std::uint64_t>(
        ElementWiseBenchmarkDescription::DefaultKeySwitchColumns,
        "KeySwitchColumns");
    default_workload_params.add<std::uint64_t>(
        ElementWiseBenchmarkDescription::DefaultPtxtPrimeModulus,
        "PtxtPrimeModulus");
    default_workload_params.add<std::uint64_t>(
        ElementWiseBenchmarkDescription::DefaultHenselLifting, "HenselLifting");

    default_workload_params.add<std::uint64_t>(
        ElementWiseBenchmarkDescription::DefaultNumThreads, "NumThreads");
...
hebench::cpp::BaseBenchmark *ElementWiseBenchmarkDescription::createBenchmark(
...
void ElementWiseBenchmarkDescription::destroyBenchmark(
...
std::string ElementWiseBenchmarkDescription::getBenchmarkDescription(
    const hebench::APIBridge::WorkloadParams *p_w_params) const
{
    ...
    std::uint64_t key_switch_columns =
        p_w_params
            ->params[ElementWiseBenchmarkDescription::Index_KeySwitchColumns]
            .u_param;
    std::uint64_t ptxt_prime_modulus =
        p_w_params
            ->params[ElementWiseBenchmarkDescription::Index_PtxtPrimeModulus]
            .u_param;
    std::uint64_t helsel_lifting =
        p_w_params->params[ElementWiseBenchmarkDescription::Index_HenselLifting]
            .u_param;

    std::uint64_t num_threads =
        p_w_params->params[ElementWiseBenchmarkDescription::Index_NumThreads]
            .u_param;
    ...
    m_w_params(bench_params)
{
    ...
    std::uint64_t key_switch_columns = m_w_params.get<std::uint64_t>(
        ElementWiseBenchmarkDescription::Index_KeySwitchColumns);
    std::uint64_t ptxt_prime_modulus = m_w_params.get<std::uint64_t>(
        ElementWiseBenchmarkDescription::Index_PtxtPrimeModulus);
    std::uint64_t helsel_lifting = m_w_params.get<std::uint64_t>(
        ElementWiseBenchmarkDescription::Index_HenselLifting);

    m_num_threads             = static_cast<int>(m_w_params.get<std::uint64_t>(
        ElementWiseBenchmarkDescription::Index_NumThreads));
...
hebench::APIBridge::Handle ElementWiseBenchmark::encode(
...
void ElementWiseBenchmark::decode(
    ...
    hebench::APIBridge::DataPackCollection *p_native)
{
    ...
    {
        int64_t *output_location = reinterpret_cast<int64_t *>(
            p_native->p_data_packs[0].p_buffers[result_i].p);
        std::vector<int64_t> result_vec;
        result_vec =
            m_p_ctx_wrapper->decodeBGV(params[result_i], m_plaintext_prime_modulus);
...
ElementWiseBenchmark::encrypt(hebench::APIBridge::Handle encoded_data)
{
...
hebench::APIBridge::Handle
ElementWiseBenchmark::decrypt(hebench::APIBridge::Handle encrypted_data)
{
...
hebench::APIBridge::Handle
ElementWiseBenchmark::load(const hebench::APIBridge::Handle *p_local_data,
                           uint64_t count)
{
    if (count != 1)
...
hebench::APIBridge::Handle ElementWiseBenchmark::operate(
            ...
            try
            {
                if (!p_ex)
                {
                    ...
                    case hebench::APIBridge::EltwiseAdd:
                        r = m_p_ctx_wrapper->evalAdd(p0, p1);
                        break;
                    ...
                    default:
                        throw hebench::cpp::HEBenchError(
                            HEBERROR_MSG_CLASS("Operation not implemented."),
                            HEBENCH_ECODE_INVALID_ARGS);
                    } // end switch
                } // end if
            }
