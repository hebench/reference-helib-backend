    ...
    hebench::APIBridge::Category category)
{
    ...
    case hebench::APIBridge::Category::Latency:
        m_descriptor.cat_params.min_test_time_ms                = 0; // flexible = 0
        m_descriptor.cat_params.latency.warmup_iterations_count = 1;
        break;

    case hebench::APIBridge::Category::Offline:
        m_descriptor.cat_params.offline.data_count[0] = 5;
    ...
    default_workload_params.add<std::uint64_t>(
        DotProductBenchmarkDescription::DefaultPolyModulusDegree, "PolyModulusDegree");
    default_workload_params.add<std::uint64_t>(
    ...
    default_workload_params.add<std::uint64_t>(
        ...
        "KeySwitchColumns");
    default_workload_params.add<std::uint64_t>(
        DotProductBenchmarkDescription::DefaultPtxtPrimeModulus,
        "PtxtPrimeModulus");
    default_workload_params.add<std::uint64_t>(
        DotProductBenchmarkDescription::DefaultHenselLifting, "HenselLifting");
    ...
    const hebench::APIBridge::WorkloadParams *p_w_params) const
{
    ...
    std::uint64_t poly_modulus_degree =
        p_w_params->params[DotProductBenchmarkDescription::Index_PolyModulusDegree]
            .u_param;
    ...
    std::uint64_t ptxt_prime_modulus =
        p_w_params->params[DotProductBenchmarkDescription::Index_PtxtPrimeModulus]
            .u_param;
    std::uint64_t helsel_lifting =
        p_w_params->params[DotProductBenchmarkDescription::Index_HenselLifting]
            .u_param;
    ...
    m_w_params(bench_params)
{
    ...
    std::uint64_t ptxt_prime_modulus = m_w_params.get<std::uint64_t>(
        DotProductBenchmarkDescription::Index_PtxtPrimeModulus);
    std::uint64_t helsel_lifting = m_w_params.get<std::uint64_t>(
        DotProductBenchmarkDescription::Index_HenselLifting);
    ...
    const hebench::APIBridge::DataPackCollection *p_parameters)
{
    ...
    values.resize(m_p_ctx_wrapper->getSlotCount());
...
DotProductBenchmark::decrypt(hebench::APIBridge::Handle encrypted_data)
{
    ...
    for (unsigned int res_count = 0; res_count < encrypted_data_ref.size();
         ++res_count)
    {
        plaintext_data.push_back(m_p_ctx_wrapper->decrypt(encrypted_data_ref[res_count])); // check if decrypt takes in const
    }
             ...
             result_x++)
        {
            try
            {
                if (!p_ex)
                {
                    ...
                    // r.reLinearize(); -> automatic in HElib (with raw mult. there is no relin)
